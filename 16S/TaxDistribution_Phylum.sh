...
# **输入文件**:
# 
# 丰度表：分类、样本名、丰度、项目名class.Bac.Composition.abundance.table.txt
# 
...
# **输出文件**：
# 
# Bacteria_composition_Class.txt，添加宿主、项目、样本统计数
# 
#     Name	Taxonomy	Hosts	Bioprojects	Samples
#     Chlorobia	Bacteria;Chlorobi;Chlorobia	23	21	173
...
# 1. 预处理：筛选丰度大于0的值
    t=Bacteria.Composition.Phylum.abundance.table.txt
    head $t
    wc -l $t # 统计共有多少行
...
# **输入文件**：
# Bacteria.Composition.Phylum.abundance.table.txt_1.txt, or ${t}_1.txt
#
#   Abditibacteriota
#   Acidobacteria
#   Actinobacteria

# ../RDP_taxonomy/rdp_16s_v18_Phylum.txt 
# 
#   Aenigmarchaeota Archaea;Aenigmarchaeota
#   Crenarchaeota   Archaea;Crenarchaeota
#   Diapherotrites  Archaea;Diapherotrites

# **输出文件**: ${t}_2.txt
#
#   Abditibacteriota        Bacteria;Abditibacteriota
#   Acidobacteria   Bacteria;Acidobacteria
#   Actinobacteria  Bacteria;Actinobacteria

    # 提取本次所需
    awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$2} NR>FNR{print $1,a[$1]}' ../RDP_taxonomy/rdp_16s_v18_Phylum.txt ${t}_1.txt > ${t}_2.txt
    head ${t}_2.txt
    wc -l ${t}_2.txt
...
# 6. 合并5列数据
    ...
    wc -l ${t}_*
    # 制作表头
    echo -e "Phylum\tTaxonomy\tHosts\tBioprojects\tSamples" > TaxaDistribution/Bacteria_TaxDistribution_Phylum.txt
    # 添加内容
    paste ${t}_2.txt ${t}_3.txt ${t}_4.txt ${t}_5.txt | cut -f1,2,4,6,8 >> TaxaDistribution/Bacteria_TaxDistribution_Phylum.txt
    head -n3 TaxaDistribution/Bacteria_TaxDistribution_Phylum.txt
  
## 保存其他过程文件
# 其中Phylum.Bac.tax.list.txt 放在Tax_list文件夹
# 把${t}_host.txt文件去掉name列没有的行，并重命名表头, 并重命名成Phylum.Bac.tax.Host.txt
   echo -e "Phylum\tSample\tAbundance\tProject_Accession\tName" > Phylum.Bac.tax.Host.txt
   ggrep -v -P '\t$' ${t}_host.txt | sort -k5,5 >> Phylum.Bac.tax.Host.txt
   head Phylum.Bac.tax.Host.txt
   
# 对${t}_2.txt文件加上表头，并重命名成Phylum.Bac.tax.txt
   echo -e "Phylum\tTaxonomy" > Phylum.Bac.tax.txt
   cat ${t}_2.txt >> Phylum.Bac.tax.txt
#把Phylum.Bac.tax.txt文件第二列拆分，并重命名
    mkdir -p Tax_list
    echo -e "Taxonomy\tKingdom\tPhylum" > Tax_list/Phylum.Bac.tax.list.txt
    sed 's/;/\t/g' Phylum.Bac.tax.txt | tail -n+2 >> Tax_list/Phylum.Bac.tax.list.txt
    head Tax_list/Phylum.Bac.tax.list.txt
